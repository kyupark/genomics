...
#include "../../../src/api/BGZF.h"
#include "../../../src/api/internal/BamStandardIndex_p.h"
#include "../../../src/api/internal/BamToolsIndex_p.h"
...
bool BamReaderPrivate::GetNextAlignmentBasic(BamAlignment& bAlignment) {
	if (GetNextAlignmentCore(bAlignment)) {
		// Shamelessly copy/pasted from BuildCharData().
//		int seqDataOffset = bAlignment.SupportData.QueryNameLength + (bAlignment.SupportData.NumCigarOperations * 4);
//		int qualDataOffset = seqDataOffset + (bAlignment.SupportData.QuerySequenceLength+1)/2;
		const char* allCharData = bAlignment.SupportData.AllCharData.data();
//		const char* qualData = ((const char*)allCharData) + qualDataOffset;
...
void BamReaderPrivate::GetBlockOffsets(std::vector<BlockOffset>& blockOffsets, bool no_mapped) {
	BamStandardIndex* std_index = dynamic_cast<BamStandardIndex*>(Index);
	std_index->SetCacheMode(BamIndex::FullIndexCaching);

	BamStandardIndexData& the_index_offsets = std_index->m_indexData;
	BamAlignment bAlignment;
	if(!no_mapped) {
		for(BamStandardIndexData::const_iterator itr = the_index_offsets.begin(); the_index_offsets.end() != itr; ++itr) {
			int64_t max_id = itr->second.Offsets.size();
	//		cout << "O: " << itr->first << ": " << max_id << "\n";
			if(max_id < 1) {
				continue;
			}
			if(10 < max_id) {
				int64_t o_id = 0;
				for(; o_id < max_id;) {
					int64_t the_pos = itr->second.Offsets[o_id];
					if(blockOffsets.empty() && 0 == the_pos) {
						if(!LoadNextAlignmentCore(bAlignment)) {
							BlockOffset an_offset;
							an_offset.ref_id = bAlignment.RefID;
							an_offset.offset = the_pos;
							an_offset.position = bAlignment.Position;

							blockOffsets.push_back(an_offset);
							o_id += 512;
							continue;
						}
					}
					if(0 == the_pos || !mBGZF.Seek(the_pos)) {
						++o_id;
						continue;
					}
	//				cout << "here-0: the pos: " << the_pos << "\n";
					//				cout << "here-1: load alignment\n";
					if(!LoadNextAlignmentCore(bAlignment)) {
						o_id += 512;
						continue;
					}
	//				cout << "here-2: alignments\n";
	//				cout << the_pos << "\t" << bAlignment.Name << "\t" << bAlignment.RefID << "\t" << bAlignment.Position << "\n";
					BlockOffset an_offset;
					an_offset.ref_id = bAlignment.RefID;
					an_offset.offset = the_pos;
					an_offset.position = bAlignment.Position;
				...
				if(o_id != (max_id - 1)) {
	//				cout << "here-3: the last element\n";
					int64_t the_pos = itr->second.Offsets.back();
					if(0 != the_pos || !mBGZF.Seek(the_pos)) {
						if(LoadNextAlignmentCore(bAlignment)) {
	//					cout << the_pos << "\t" << bAlignment.Name << "\t" << bAlignment.RefID << "\t" << bAlignment.Position << "\n";
							BlockOffset an_offset;
	...
	//			cout << "here-4: small number of elements\n";
				for(int64_t o_id = 0; o_id < max_id; ++o_id) {
					int64_t the_pos = itr->second.Offsets[o_id];
					if(0 != the_pos || !mBGZF.Seek(the_pos)) {
						if(LoadNextAlignmentCore(bAlignment)) {
	//					cout << the_pos << "\t" << bAlignment.Name << "\t" << bAlignment.RefID << "\t" << bAlignment.Position << "\n";
...
//		cout << "here-0\n";
		BamStandardIndexData::reverse_iterator itr = the_index_offsets.rbegin();
		for(;the_index_offsets.rend() != itr;++itr) {
			if(itr->second.Offsets.size() > 0) {
				break;
			}
		}
		int64_t max_id = itr->second.Offsets.size();
		--max_id;
//		cout << "here-1: " << max_id << "\n";
		for(int64_t o_id = max_id; o_id >=0; --o_id) {
			int64_t the_pos = itr->second.Offsets[o_id];
			if(0 == the_pos || !mBGZF.Seek(the_pos)) {
				continue;
			}
	...
	int64_t the_pos = mBGZF.Tell();
	int64_t the_prev_pos = the_pos;
//	cout << "Mapped remaining\n";
	BamAlignment prev_alignment;
	while(LoadNextAlignmentCore(bAlignment)) {
		if(-1 == bAlignment.RefID) {
			break;
		}
		the_pos = the_prev_pos;
		the_prev_pos = mBGZF.Tell();
		prev_alignment = bAlignment;
//		cout << the_prev_pos << "\t" << bAlignment.Name << "\t" << bAlignment.RefID << "\t" << bAlignment.Position << "\n";
	}

	BlockOffset an_offset;
	an_offset.ref_id = prev_alignment.RefID;
	an_offset.offset = the_prev_pos;
	an_offset.position = prev_alignment.Position;
	blockOffsets.push_back(an_offset);
	mBGZF.Seek(the_prev_pos);
	cout << "[BamReaderPrivate.GetBlockOffsets] Unmapped region: " << the_prev_pos << "\n";
//	const int64_t a_block_size = static_cast<int64_t>(1) << static_cast<int64_t>(40);
	const int64_t a_block_entries = 256 * 1024;
	int64_t n_total_entries = 0;
	int64_t n_entries = 0;
	vector<char> buffer;
	while(ReadNextAlignment(buffer)) {
		int64_t cur_pos = mBGZF.Tell();
//		int64_t delta = cur_pos - the_pos;
		++n_entries;
		++n_total_entries;
		if(n_entries == a_block_entries) {
		if(LoadNextAlignmentCore(bAlignment)) {
			++n_entries;
			++n_total_entries;
	//			cout << cur_pos << "\n";
				cout << "[BamReaderPrivate.GetBlockOffsets] Processed: " << n_total_entries << "\n";
	//			cout << cur_pos << "\t" << bAlignment.Name << "\t" << bAlignment.RefID << "\t" << bAlignment.Position << "\t" << n_entries << "\n";
				BlockOffset an_offset;
				an_offset.ref_id = bAlignment.RefID;
				an_offset.offset = cur_pos;
				an_offset.position = bAlignment.Position;
				blockOffsets.push_back(an_offset);
				the_pos = cur_pos;
				n_entries = 0;
	...
	an_offset.ref_id = -1;
	an_offset.offset = numeric_limits<int64_t>::max();
	an_offset.position = -1;
	blockOffsets.push_back(an_offset);
	cout << "[BamReaderPrivate.GetBlockOffsets] Total Processed: " << n_total_entries << "\n";
}

void BamReaderPrivate::GetUnmappedBlockOffsets(std::vector<BlockOffset>& blockOffsets) {
	BamStandardIndex* std_index = dynamic_cast<BamStandardIndex*>(Index);
	std_index->SetCacheMode(BamIndex::FullIndexCaching);
...
//				cout << "here-3: the last element\n";
				int64_t the_pos = itr->second.Offsets.back();
				if(0 != the_pos || !mBGZF.Seek(the_pos)) {
//					LoadNextAlignmentCore(bAlignment);
//					cout << the_pos << "\t" << bAlignment.Name << "\t" << bAlignment.RefID << "\t" << bAlignment.Position << "\n";
//					BlockOffset an_offset;
//					an_offset.ref_id = bAlignment.RefID;
//					an_offset.offset = the_pos;
...
//		cout << the_prev_pos << "\t" << bAlignment.Name << "\t" << bAlignment.RefID << "\t" << bAlignment.Position << "\n";
	...
	blockOffsets.push_back(an_offset);
	mBGZF.Seek(the_prev_pos);
//	cout << "Unmapped region: " << the_prev_pos << "/" << the_pos << "\n";
//	const int64_t a_block_size = static_cast<int64_t>(1) << static_cast<int64_t>(40);
	const int64_t a_block_entries = 2 * 1024 * 1024;
	int64_t n_entries = 0;
	vector<char> buffer;
	while(LoadNextAlignmentCore(bAlignment)) {
		int64_t cur_pos = mBGZF.Tell();
...
bool BamReaderPrivate::ReadNextAlignment(std::vector<char>& allCharData) {
	// read in the 'block length' value, make sure it's not zero
	char buffer[4];
	mBGZF.Read(buffer, 4);
	uint32_t BlockLength = BgzfData::UnpackUnsignedInt(buffer);
...
//	char* allCharData = (char*)calloc(sizeof(char), dataLength);
//	allCharData = (char*)realloc(allCharData, sizeof(char) * dataLength);
	allCharData.resize(dataLength);

	if ( mBGZF.Read(&allCharData[0], dataLength) == (signed int)dataLength) {
	// set success flag
	readCharDataOK = true;
	}
...
// populates BamAlignment with alignment data under file pointer, returns success/fail
bool BamReaderPrivate::LoadNextAlignmentWithName(BamAlignment& bAlignment) {
	bool results = LoadNextAlignment(bAlignment);
	if(!results) {
		return false;
	}
	// Shamelessly copy/pasted from BuildCharData().
...
// populate BamAlignment without concerning overlapped Region, returns success/fail
bool BamReaderPrivate::LoadNextAlignmentCore(BamAlignment& bAlignment) {
        if(LoadNextAlignment(bAlignment)) {
		return BuildCharData(bAlignment);
	}
	return false;
