...
class API_EXPORT BamReader {
	...
	//   default behavior is to prefer the BamToolsIndex (*.bti) if both are available
        ...
        BgzfData& GetBGZF();
        void GetBlockOffsets(std::vector<BlockOffset>& blockOffsets);
        void GetUnmappedBlockOffsets(std::vector<BlockOffset>& blockOffsets);
	...
	// (refId/position, alignment flags, CIGAR, mapQuality, etc)
        ...
        // GetNextAlignmentCore + Only ReadName
        bool GetNextAlignmentBasic(BamAlignment& bAlignment);
	// SAME AS *Core, except the query name is filled in
        bool GetNextAlignmentWithName(BamAlignment& bAlignment);
	// Non regionChecking
	bool LoadNextAlignment(BamAlignment& bAlignment);
	// SAME AS the above, except the query name is filled in
	bool LoadNextAlignmentWithName(BamAlignment& bAlignment);
	// with base chars
	bool LoadNextAlignmentCore(BamAlignment& bAlignment);
	bool ReadNextAlignment(std::vector<char>& allCharData);
