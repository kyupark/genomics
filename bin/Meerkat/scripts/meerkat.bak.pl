...
&discord($bamfile, $prefix, $is_files_dir, $is_actual_dir, $blacklistfile, $clip, $bamreader_command, $sd_cutoff_disc, $isinfofile, $samtools_command, $remove_dup, $black_rg) if ($discordant);
my $time  = time;
my $local = localtime($time);
print STDERR "$local extracted discordant reads\n" if ($discordant);

&cluster($prefix, $is_files_dir, $is_actual_dir, $blacklistfile, \%is, $cut_sr, $ad_align, $alt_map_max, $alt_map_max_clip, $clip, $samtools_command, $cl, $bamreader_command) if ($cluster);
$time  = time;
$local = localtime($time);
print STDERR "$local called discordant clusters\n" if ($cluster);

&mpd($prefix, $is_files_dir, $is_actual_dir, $bamreader_command, $sd_cutoff_disc, $support_mps, $support_mpf) if ($mpd);
$time  = time;
$local = localtime($time);
print STDERR "$local called events by read pairs\n" if ($mpd);

&alg($bamreader_command, $prefix, $is_files_dir, $is_actual_dir, $sd_cutoff_disc, $support_mps, $support_mpf, $remove_dup, $clip, $cut_sr, $reference_path) if ($alg);
$time  = time;
$local = localtime($time);
print STDERR "$local aligned split reads\n" if ($alg);

&srd($bamreader_command, $prefix, $is_files_dir, $is_actual_dir, $sd_cutoff_disc, $support_mps, $support_mpf, $remove_dup, $clip, $reference_path, $cut_sr, $support_reads) if ($srd);
$time  = time;
$local = localtime($time);
print STDERR "$local confirmed events by split reads\n" if ($srd);

&filter($bamreader_command, $prefix, $is_files_dir, $is_actual_dir) if ($filter);
$time  = time;
$local = localtime($time);
print STDERR "$local filtered events\n" if ($filter);
&blast_bp($bamreader_command, $prefix, $is_files_dir, $is_actual_dir, $sd_cutoff_disc, $support_mps, $support_mpf, $remove_dup, $clip, $reference_path, $cut_sr, $support_reads) if ($blast_bp);
$time  = time;
$local = localtime($time);
...
sub alg {
	...
	my $cut_sr            = shift;
	my $reference_path    = shift;
	print("$bamreader_command alg -x $prefix -D $is_actual_dir -d $sd_cutoff_disc -p $support_mps -o $support_mpf -m $remove_dup -l $clip -s $cut_sr -F $reference_path\n");
	system("$bamreader_command alg -x $prefix -D $is_actual_dir -d $sd_cutoff_disc -p $support_mps -o $support_mpf -m $remove_dup -l $clip -s $cut_sr -F $reference_path");
}
...
sub discord {
	...
	if ($blacklistfile) {
		if ($remove_dup) {
			print("$bamreader_command dre -t $threads_bwa -D $is_actual_dir -f -v -P -s $sd_cutoff_disc -b $blacklistfile -r $black_rg -i $isinfofile -o $disc_bam -d $dup_file -a $bamfile >$drelog\n");
			system("$bamreader_command dre -t $threads_bwa -D $is_actual_dir -f -v -P -s $sd_cutoff_disc -b $blacklistfile -r $black_rg -i $isinfofile -o $disc_bam -d $dup_file -a $bamfile >$drelog");
			my $time  = time;
			my $local = localtime($time);
			print STDERR "$local extracted discordant read pairs\n";
			if ($clip) {
				print("$bamreader_command dre -t $threads_bwa -D $is_actual_dir -f -v -P -s $sd_cutoff_disc -b $blacklistfile -r $black_rg -i $isinfofile -o $disc_cl_bam -d $dup_cl_file -a $cl_bam >>$drelog\n");
				system("$bamreader_command dre -t $threads_bwa -D $is_actual_dir -f -v -P -s $sd_cutoff_disc -b $blacklistfile -r $black_rg -i $isinfofile -o $disc_cl_bam -d $dup_cl_file -a $cl_bam >>$drelog");
				my $time  = time;
				my $local = localtime($time);
				print STDERR "$local extracted discordant read pairs from remapped bam file\n";
			}
		} else {
			print("$bamreader_command dre -t $threads_bwa -D $is_actual_dir -f -v -m -s $sd_cutoff_disc -b $blacklistfile -r $black_rg -i $isinfofile -o $disc_bam -a $bamfile >$drelog\n");
			system("$bamreader_command dre -t $threads_bwa -D $is_actual_dir -f -v -m -s $sd_cutoff_disc -b $blacklistfile -r $black_rg -i $isinfofile -o $disc_bam -a $bamfile >$drelog");
			my $time  = time;
			my $local = localtime($time);
			print STDERR "$local extracted discordant read pairs\n";
			if ($clip) {
...
#	goto AS2 if ($cl == 2 or $cl == 3);
	...
	print("$bamreader_command r_alt -t $threads_bwa -x $prefix -D $is_actual_dir -i $disc_bam -z $raw_alt_map_file\n");
	system("$bamreader_command r_alt -t $threads_bwa -x $prefix -D $is_actual_dir -i $disc_bam -z $raw_alt_map_file");
	if ($clip) {
		system("$bamreader_command r_alt -t $threads_bwa -x $prefix -D $is_actual_dir -i $disc_cl_bam -z $raw_alt_map_cl_file\n");
		system("$bamreader_command r_alt -t $threads_bwa -x $prefix -D $is_actual_dir -i $disc_cl_bam -z $raw_alt_map_cl_file");
