...
my %opts = (D=> 'null', k=>500, r=>1000, n=>10000, l=>1, t=>1, q=>15, c=>35, s=>20, u=>0, f=>100, N=>5, R=>'null', P=>'all');
getopts("A:b:c:C:D:f:I:k:l:Mn:N:P:q:r:R:s:S:t:u:W:h", \%opts);
my $tmp_dir = $opts{D};
...
if ($is) {
	...
	if("null" eq $tmp_dir) {
		$preprocess_cmd = "$bamreader_command preprocess -t $threads_bwa -F $bwa_index -l $remap_cl -b $blacklist_cutoff -x $prefix -r $black_rg -c $clip_cut -s $sr_cut -n $number_reads -q $qual_trim -N $numberofn -f $n_alter_map ";
	}
