...
if ($is) {
	my $preprocess_cmd = "$bamreader_command preprocess -t $threads_bwa -F $bwa_index -l $remap_cl -b $blacklist_cutoff -x $prefix -r $black_rg -c $clip_cut -s $sr_cut -n $number_reads -q $qual_trim -N $numberofn -f $n_alter_map ";
	if ($uupair) {
